...
(function () {
  ...
  var TOOL_INFO = [
    ...
    { id: 'brand-color', badge: '◐', name: 'Брэндийн өнгө', what: 'Салбартаа тохирсон 5 өнгийн палитр — лого, пост, вэб дээр шууд харна.',
      when: 'Шинэ брэнд, лого, сошиалын өнгө тогтоох', href: '/tools/brand-color/', go: 'Нээх →' },
    { id: 'templates', badge: '▦', name: 'Монгол загварууд', what: 'Цагаан сар, Наадам, ажлын зар, хямдрал зэрэг бэлэн сошиал загвар.',
      when: 'Баярын мэндчилгээ, зар, постыг хурдан бэлдэх', href: '/tools/templates/', go: 'Нээх →' }
  ];
    ...
    return TOOL_INFO.filter(function (t) { return t.id !== skip; }).map(function (t) {
      ...
      return (
        '<a class="tp-card" href="' + toolHref(t) + '">' +
          ...
          '<span class="tp-go">' + (t.go || (t.href ? 'Засварлагч нээх ↗' : 'Нээх →')) + '</span>' +
        '</a>'
      );
    }).join('');
  }
  ...
  function toolPicker() {
    ...
    return (
      '<section class="sec picker" id="picker">' +
        ...
        '<div class="tp-grid reveal">' + cards + '</div>' + pdfTasks() +
  ...
  // search landing pages (/tools/<slug>/, generated by .github/scripts/landings.py)
  var PDF_TASKS = [["pdf-to-word", "PDF → Word"], ["word-to-pdf", "Word → PDF"], ["pdf-to-excel", "PDF → Excel"], ["pdf-to-ppt", "PDF → PowerPoint"], ["pdf-to-jpg", "PDF → JPG"], ["jpg-to-pdf", "Зураг → PDF"], ["translate-pdf", "PDF орчуулах"], ["merge-pdf", "PDF нэгтгэх"], ["split-pdf", "PDF задлах"], ["compress-pdf", "PDF шахах"], ["sign-pdf", "Гарын үсэг"], ["protect-pdf", "Нууц үг тавих"], ["unlock-pdf", "Нууц үг арилгах"], ["pdf-ocr", "Текст таних (OCR)"], ["edit-pdf", "PDF засах"], ["fill-pdf", "Маягт бөглөх"], ["pdf-page-numbers", "Хуудасны дугаар"], ["watermark-pdf", "Усан тэмдэг"], ["summarize-pdf", "PDF хураангуйлах"]];
  function pdfTasks() {
    return '<nav class="pdf-tasks" aria-label="PDF үйлдлүүд"><span class="flow-t">PDF үйлдлүүд</span>' +
      PDF_TASKS.map(function (t) { return '<a href="/tools/' + t[0] + '/">' + esc(t[1]) + '</a>'; }).join('') + '</nav>';
  }
  function otherTools(id) {
    return (
      '<section class="sec other-tools">' +
        '<div class="sec-meta"><span>БУСАД ХЭРЭГСЭЛ</span><span>MORE TOOLS +</span></div>' +
        '<div class="tp-grid">' + toolCards(id) + '</div>' + pdfTasks() +
      '</section>'
    );
  }
